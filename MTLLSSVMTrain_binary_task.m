function [score1,error_opt,error_th,error_emp,alpha2, b,score_th,variance_th,score_emp,var_emp,y_opt,pred1,obj1] = MTLLSSVMTrain_binary_task(trnXs,trnXt,trnY, gamma, lambda,M,Ct,tstX1,nt,centered,k,n_test)
...
% score_mean1=(1/(k*p))*mean(tstX1(:,1:1135),2)'*A^(1/2)*Qtildez*A^(1/2)*Mdelta1*J'*(trnY(:)-P*b)+[b(1);b(1);b(2);b(2)];
% score_mean2=(1/(k*p))*mean(tstX1(:,1135+1:end),2)'*A^(1/2)*Qtildez*A^(1/2)*Mdelta1*J'*(trnY(:)-P*b)+[b(1);b(1);b(2);b(2)];
for task=1:k-1 
    X1{task}=trnXs{task};
end
 X1{k}=trnXt;
...
%inr=((score_mean_test)/(4*2*(abs(variance_test1(3)))));
 ...
 in3_1=0.5*erfc(((score_emp(1)-score_emp(2))/(2*sqrt(2)*sqrt(abs(var_emp(1))))))
 in4_1=0.5*erfc(((score_emp(1)-score_emp(2))/(2*sqrt(2)*sqrt(abs(var_emp(2))))))
 ...
 moy=(score_th(m*(k-1)+1)+score_th(m*(k-1)+2))/2;
% moy=(score_emp(1)+score_emp(2))/2;
