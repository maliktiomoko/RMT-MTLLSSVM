function [score1,error_opt,alpha2, b,score_th,variance_th,score_emp,var_emp,y_opt,covar,obj2,pred1] = MLSSVRTrain_th1_centered_other_class(trnXs,trnXt,trnY, gamma, lambda,M,Ct,tstX1,nt,centered,k,n_test,tl)
...
%         obj=@(vt) obj(vt)+exp(((Matm*vt)^2)./(8*vt'*Mat(:,:,m*(k-1)+j)*vt));
         obj=@(vt) max(obj(vt),0.5*erfc((((Matm*vt)./sqrt(8*vt'*Mat(:,:,m*(k-1)+j)*vt)))));
%         obj=@(vt) obj(vt)+0.5*erfc(sqrt(((Matm*vt)^2)./(8*vt'*Mat(:,:,m*(k-1)+j)*vt)));
%      obj=@(vt) obj(vt)-((Matm*vt)^2)./(8*vt'*Mat(:,:,m*(k-1)+j)*vt);
end
%   obj=@(vt) -(1/(m-1))*log(obj(vt));
% obj=@(vt) -obj(vt);
%Mat=(1./(tildeD(i)))*Gamma*(diag(kron(KAPPA(:,i),ones(2,1))./cbar)+Vc)*Gamma;
 ...
 er=zeros(m*k,1);er(m+tl)=1;
 Mat2=er'*(eye(m*k)-diag(deltabar)^(-1/2)*Gamma*diag(deltabar)^(1/2)*Ze);
 %[obj1,error_opt]=fmincon(obj,x0,[],[],[Mat2],[1]);
 [obj1,error_opt]=fmincon(obj,x0,[],[],[],[]);
 yopt=pinv(Ze)*(obj1);
%  yopt=ytilde;
%  y_opt=y_opt./norm(y_opt);
   ez=zeros(k*m,1);ez(m+tl)=1;er=[0;1];
   meanf =@(sol) ((yopt+kron(er,sol*ones(m,1)))-diag(deltabar)^(-1/2)*Gamma*diag(deltabar)^(1/2)*Ze*(yopt+kron(er,sol*ones(m,1))))'*ez;
   asolve=fsolve(meanf,1);
   y_opt=yopt+kron(er,asolve*ones(m,1));
  obj2=obj1./norm(obj1);
 
 init_ordero=1:m*k;
init_order=1:m*k;
%covar(1)=variance_th(m*(k-1)+1);
covar=zeros(m,m);
for kc=1:m
        ytildevar1=-ones(m*k,1);
        ytildevar1(kc)=1;ytildevar1(kc+m)=1;
        bn(1)=(nt(1:m)./sum(nt(1:m)))'*ytildevar1(1:m);
        bn(2)=(nt(m+1:end)./sum(nt(1+m:end)))'*ytildevar1(m+1:end);
        ytildevarO=ytildevar1-[bn(1)*ones(m,1);bn(2)*ones(m,1)];
    for j=1:m
%         ytildevar=circshift(ytilde,j);
%          if j~=1
%               init_order(1)=m*(k-1)+j;
%               init_order(j)=m*(k-1)+j-1;
%               init_order(m*(k-1)+1)=m*(k-1)+j;
%               init_order(m*(k-1)+j)=m*(k-1)+j-1;
%          end
%         nsn=nt(init_order);
        ytildevar=-ones(m*k,1);
        ytildevar(j)=1;ytildevar(j+m)=1;
        bn(1)=(nt(1:m)./sum(nt(1:m)))'*ytildevar(1:m);
        bn(2)=(nt(m+1:end)./sum(nt(1+m:end)))'*ytildevar(m+1:end);
        ytildevar0=ytildevar-[bn(1)*ones(m,1);bn(2)*ones(m,1)];
        MGM=Mgota'*Qtilde0*V(:,:,m*(k-1)+tl)*Qtilde0*Mgota;
    %         MGM(1:2,1:2)=MGMc1(1:2,1:2);MGM(3:4,3:4)=MGMc1(3:4,3:4);
        Mat=diag(sqrt(are))*Gamma*(MGM+diag(kappa(m*(k-1)+tl,:)./are'))*Gamma*diag(sqrt(are));
            %variance_th(m*(i-1)+j)=
        covar(kc,j)=ytildevarO'*Mat*ytildevar0;
    end
end
