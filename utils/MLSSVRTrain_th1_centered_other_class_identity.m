function [score1,error_opt,alpha2, b,score_th,variance_th,score_emp,var_emp,y_opt,covar,obj2] = MLSSVRTrain_th1_centered_other_class_identity(trnXs,trnXt,trnY, gamma, lambda,M,tstX1,nt,centered,k,n_test,tl)
...
for i=1:k-1
    for j=1:m
        d=zeros(k,1);d(i)=1;d=d*d';
         C(:,:,m*(i-1)+j)=A^(1/2)*(kron(d,eye(p)+M(:,m*(i-1)+j)*M(:,m*(i-1)+j)'))*A^(1/2);
%         C(:,:,m*(i-1)+j)=(k*p)*A^(1/2)*(kron(d,X11{j,i}'*X11{j,i}/nt(m*(i-1)+j)))*A^(1/2);
    end
end
    for j=1:m
        d=zeros(k,1);d(k)=1;d=d*d';
         C(:,:,m*(k-1)+j)=A^(1/2)*(kron(d,eye(p)+M(:,m*(k-1)+j)*M(:,m*(k-1)+j)'))*A^(1/2);
